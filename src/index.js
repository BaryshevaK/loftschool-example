...
function delegate(target, fn) {
    var countChildTarget = target.childNodes.length;

    for (let i = 0; i < countChildTarget; i++) {
        if (target.childNodes[i].tagName === 'BUTTON') {
            target.childNodes[i].addEventListener('click', fn);
        }
    }
}
...
function once(target, fn) {
    ...
    // target.onclick = () => target.removeEventListener('click', fn);
}
