...
/*
    const elementWhat = document.createElement('div');
    document.body.appendChild(elementWhat);
    elementWhat.id = 'one';

    const elementWhere = document.createElement('div');
    document.body.appendChild(elementWhere);
    elementWhere.id = 'two';
*/
/*
 Задание 3:
...
function findAllPSiblings(where) {
    var countElemBody = where.children.length;
    var array = [];

    for (let i = 0; i < countElemBody - 1; i++) {
...
function deleteTextNodes(where) {
    var countElemBoby = where.childNodes.length;

    for (let i = 0; i < countElemBoby; i++) {
...
function deleteTextNodesRecursive(where) {
    ...
    for (let i = 0; i < countElemBoby; i++) {
        if (where.childNodes[i].childNodes.length !== undefined) {
